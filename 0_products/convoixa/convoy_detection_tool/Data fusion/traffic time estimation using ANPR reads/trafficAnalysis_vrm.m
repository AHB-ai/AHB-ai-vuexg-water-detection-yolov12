...
% certain time of the day given a vrm.
% INPUT:
...
% second column is the travel time in seconds. The third column stores two
% camera names, and the fourth column is the real time.
% Haiyue@Jun 2015

function results_stats = trafficAnalysis_vrm(dir_path, vrm, date_start, date_end, time_start, time_end, ANPR1, ANPR2, output_dir)
...
if ~isempty(date_start) && ~isempty(date_end)
    ...
    for i=1:length(dateList)
        ...
        file = [dir_path '\travel time_vrm\' date '_vrm.mat'];
        load(file);
        flag_vrm = isKey(vrm_trafficMap,vrm);
        if flag_vrm==1
            val = vrm_trafficMap(vrm);
            if isempty(ANPR1) && isempty(ANPR2)
                count=1;
                key = keys(val);
                temp = {};
                count = 1;
...
if isempty(temp_results)
    if flag_wrong_vrm == 1
         disp('Error ! Search VRM does not exist, please check !');
    end
...
if ~isempty(temp_results)
    ...
    for i=1:sz(1)
        ...
        results_stats{i,4} = real_time;
    end
    clear stats;
    clear temp_results;
    output_file = [output_dir '\' vrm ' ' ANPR1 ' ' ANPR2 ' ' date_start '-' date_end '.csv'];
